...
Before using this tutorial, we recommend that you read through our [introductory tutorial](articles/introductory_tutorial.html).
...
`segmentNeighborhood = TRUE` and specify the value of `Kmax` to `segmentation()`.
This step can be time consuming. A grid search approach is also implemented in
**DiffSegR** (see `gridSearch` and `alphas` parameters of `segmentation()`).
...
$\log(\alpha)=2.1$ or $\alpha=4.28$ symbolized by the red line. Setting `alpha=4`
in `segmentation()` leads to 11 segments on the reverse strand.
...
all_models <- readRDS("DIFFSEGR_TEST/all_models_minus.rds")
ggplot2::ggplot(
  all_models,
  ggplot2::aes(
    x = log2(alphas), 
